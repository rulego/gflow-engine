...
* **Full approval semantics:** single sign-off, countersign (parallel/sequential, all/majority), dynamic add/remove signers, transfer, delegation, claim, return, withdraw, suspend/resume, overdue handling.
...
* **Rule-engine integration:** `serviceTask` calls Go functions, `automation` invokes `RuleGo` rule chains, `aiAgent` talks to agent rule chains ([rulego-components-ai](https://github.com/rulego/rulego-components-ai)), and `httpCall` performs synchronous HTTP calls.
...
* **Event hooks:** 17 task lifecycle events (started/assigned/candidateCreated/claimed/unclaimed/approved/rejected/forwarded/addSign/reduceSign/resolved/returned/withdrawn/suspended/activated/completed/terminated) dispatched after commit; register one or many listeners via the builder. Platform hooks: cross-tenant overdue scan (`ScanOverdueTasks`) and batch claimable-instance lookup (`GetClaimableInstanceIDs`).
...
func main() {
	...
	//    Dependencies are pulled from the engine itself; host service functions
	//    are registered via WithServiceFuncs (metadata feeds the designer catalog,
	//    implementations feed the runtime — see docs/components.md).
	...
	_, err = engine.GetProcessService().Deploy(ctx, admin, &model.WfProcess{
		...
		Name:           "Leave Approval",
		DefinitionJSON: leaveApprovalDSL, // process DSL JSON
		TenantID:       "default",
		CreatedBy:      "admin",
	}, true)
	if err != nil {
